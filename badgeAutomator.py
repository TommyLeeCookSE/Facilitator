...
#Main--------------------------------------------------------------------------------------------------------------  
  file = open("badgeFile.txt", "r")
  for line in file:
    if line != "\n":
      dataArray.append(line.strip())
  file.close()
  
  file = open("finalizedBadges.csv", "w")
  ...
  for i in range(len(dataArray)):
    ...
    if "Edwards ID #" in dataArray[i]:
      ...
      dataDict["EID"] = tempString[idIndex+4:idIndex+11].strip().upper()
    ...
    if "Job Title:" in dataArray[i]:
      ...
      try: #Some forms use Building some use Bldg
        bldgIndex = dataArray[i].index("Bldg:")
      except:
        bldgIndex = dataArray[i].index("Building:")
