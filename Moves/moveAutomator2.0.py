...
def moveDateFormatter(sentance):
    ...
    # print(moveDate)
    month = moveDate[moveDate.find(" "):moveDate.find(' ',moveDate.find(" ",11))].strip()
    # print(month)
    day = moveDate[moveDate.find(" ",11):moveDate.find(",",11)].strip()
    # print(day)
    year = moveDate[moveDate.find(" ",16):].strip()
    # print(year)
...
with open("Moves/readFile.csv", 'r') as file:
    ...
    for lines in reader:
        for sentance in lines:
            ...
            #TODO need to add Shared functionality, waiting on a form with a shared employee to be sent.
            # elif "If Shared - please add the name of Shared employee" in sentance:
            #     print(lines)
            #     print(sentance[sentance.find('\t')+1])
            #     fname = fname + " " + lname
            #     # print(fname)
            ...
            #TODO Add functionality that checks building, and checks first CHAR of cube to ensure that the information has been added, waiting for this error to happen again
            ...
            elif "Include any other special requests here" in sentance: #Appends the list after reaching end of the current move request, restarts loop and continues till done
